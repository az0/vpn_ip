...
def write_hostnames_to_text_file(filename, hostnames, units="hostnames"):
    """
    ...
    if len(hostnames) == 0 and not os.path.exists(filename):
        print(f'no {units} to write to {filename}')
        return
    print(f'writing {len(hostnames):,} {units} to {filename}')
